...
inline std::string exec2(std::string cmd)
{
    // pipes command result to string for use
    std::array<char, 1024> buffer;
    std::string result;
    std::unique_ptr<FILE, int (*)(FILE *)> pipe(popen(cmd.c_str(), "r"), (int (*)(FILE *))pclose);
    if (!pipe)
    {
...
inline int beldum_install(std::string &requested_package,
